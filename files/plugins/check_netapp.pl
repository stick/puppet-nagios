...
#
# $Id: check_netapp.pl,v 1.1 2007/01/25 16:09:59 cmacleod Exp $
...
if ( $variable eq 'FAN' ) {
	...
	$state = 'OK';
	$state = 'CRITICAL' if ( $snmpresponse{$snmpnvramBatteryStatus}{0} > 1 );
	...
	foreach my $key ( keys %{$snmpresponse{$snmpfilesysdfTabledfEntrydfFileSys}} ) {
		if ( defined $volume ) {
			if ( $snmpresponse{$snmpfilesysdfTabledfEntrydfFileSys}{$key} eq $volume ) {
				...
				$perfdata = sprintf("%s;%d;%d;%d;%d",$volume,$total,$used,$avail,$percent);
...
sub print_help () {
	...
	print "  -h, --help\n\tusage help\n\n";
	print_revision($PROGNAME,"\$Revision: 1.1 $PROGREVISION\$");
}
